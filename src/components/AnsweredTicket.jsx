...
const AnsweredTicket= () => {
  ...
  useEffect(() => {
    const fetchTickets = async () => {
      try {
        ...
        // Filter tickets to only show answered ones
        const answeredTickets = fetchedTickets.filter(
          (ticket) => ticket.action === "Answered"
        );
        setTickets(answeredTickets);
      } catch (err) {
  ...
  return (
    <div className="container mt-5">
      ...
      <h4 style={{color: 'blue'}}>Answered Tickets</h4>
      <table className="table table-bordered table-striped">
        ...
        <tbody>
          {tickets.length > 0 ? (
            ...
            <tr>
              <td colSpan="5">No answered tickets available.</td>
            </tr>
          )}
        </tbody>
      </table>
